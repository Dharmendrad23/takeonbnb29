...
const PropertyApprovalPage = () => {
  ...
  const fetchPendingProperties = async () => {
    try {
      const records = await listProperties({ sort: '-createdAt' });
      setProperties(records.filter((property) => property.approvalStatus === 'pending'));
    } catch (error) {
      console.error(error);
    ...
    <div className="p-6 md:p-10 max-w-7xl mx-auto w-full">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {properties.map(p => (
            <Card key={getEntityId(p)} className="cursor-pointer hover:shadow-md transition-shadow border-border" onClick={() => { setSelectedProperty(p); setIsModalOpen(true); }}>
              ...
              <CardContent className="p-4">
                ...
                <div className="mt-4 pt-4 border-t border-border flex justify-between text-sm">
                  <span className="text-muted-foreground flex items-center gap-1"><User className="w-3.5 h-3.5" /> {getHostName(p.host || p.hostId) || 'Unknown'}</span>
                  <span className="text-muted-foreground font-medium">{new Date(p.createdAt || p.created).toLocaleDateString()}</span>
                </div>
              </CardContent>
            </Card>
          ))}
          ...
          <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto bg-card">
            ...
            <div className="space-y-6">
              ...
              <div className="flex gap-2 overflow-x-auto no-scrollbar snap-x h-48">
                {(selectedProperty.photos || selectedProperty.images || []).map((photo, i) => (
                  <img key={i} src={typeof photo === 'string' ? photo : photo?.url} className="h-full w-auto object-cover rounded-lg snap-start" alt={`Property ${i}`} />
                ))}
              </div>
              ...
              <div className="border border-border p-4 rounded-xl">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <p className="flex items-center gap-2"><User className="w-4 h-4 text-muted-foreground"/> {getHostName(selectedProperty.host || selectedProperty.hostId)}</p>
                  <p className="flex items-center gap-2"><Mail className="w-4 h-4 text-muted-foreground"/> {selectedProperty.host?.email || selectedProperty.hostId?.email}</p>
                  <p className="flex items-center gap-2"><Phone className="w-4 h-4 text-muted-foreground"/> Contact info</p>
                  <p className="flex items-center gap-2"><Calendar className="w-4 h-4 text-muted-foreground"/> Member since {new Date(selectedProperty.host?.createdAt || selectedProperty.host?.created || selectedProperty.hostId?.createdAt || selectedProperty.hostId?.created || Date.now()).getFullYear()}</p>
                </div>
              </div>
