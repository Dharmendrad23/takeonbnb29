...
const HostBookingCalendar = () => {
  ...
  useEffect(() => {
    const fetchProps = async () => {
      try {
        ...
        const hostProps = records.filter(p => String(p.hostId) === String(currentUser.id));
        setProperties(hostProps);
        if (hostProps.length > 0) setSelectedPropertyId(hostProps[0]._id || hostProps[0].id);
      } catch (e) {
        console.error("Props fetch error", e);
      }
    };
    if (currentUser) fetchProps();
  }, [currentUser]);
  ...
  const fetchCalendarData = async () => {
    ...
    try {
      ...
      const allBookings = Array.isArray(bookRes.data) ? bookRes.data : bookRes.data?.items || [];
      const propBookings = allBookings.filter(b =>
        String(b.propertyId?._id || b.propertyId) === String(selectedPropertyId) &&
  ...
  const handleBlockDates = async () => {
    ...
    setIsBlocking(true);
    try {
      toast.success("Dates blocked successfully (local only — backend unavailable_dates not yet implemented)");
      const newBlock = {
        id: `block-${Date.now()}`,
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          {properties.length > 0 && (
            <Select value={selectedPropertyId} onValueChange={setSelectedPropertyId}>
              ...
              <SelectContent>
                {properties.map(p => <SelectItem key={p._id || p.id} value={p._id || p.id}>{p.title}</SelectItem>)}
              </SelectContent>
            </Select>
          )}
        </div>
