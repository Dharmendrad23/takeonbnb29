...
const HostRegisterPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      await signup(formData.email, formData.password, formData.name, 'host');
  ...
  return (
    <div className="min-h-screen bg-muted/30 flex items-center justify-center p-4">
      <Helmet><title>Become a Host | Take On BnB</title></Helmet>
      <div className="bg-card w-full max-w-md p-8 rounded-3xl shadow-hover border border-border">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground mt-1">List your property on Take On BnB</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Button type="submit" className="w-full h-12 rounded-xl text-base mt-2" disabled={isLoading}>
            {isLoading ? 'Creating...' : 'Register as Host'}
          </Button>
        </form>
