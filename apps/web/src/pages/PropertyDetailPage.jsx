...
const PropertyDetailPage = () => {
  ...
  const demoReviewTexts = [
    "Beautiful property and a very comfortable stay. Everything was clean and well maintained.",
    "Had a wonderful experience here. The location was peaceful and the host was very helpful.",
    "Really enjoyed our stay. The property was exactly as shown and the hospitality was excellent.",
    "A lovely place for a family trip. Rooms were comfortable and everything was well arranged.",
    "Great stay and excellent hospitality. Would definitely stay here again.",
    "Very peaceful property with good facilities. We had a pleasant and relaxing experience.",
    "The property was clean, comfortable and well maintained. Overall a very good stay.",
    "Amazing experience from check-in to check-out. Highly recommended for a short getaway.",
  ];

  const propertyKey = String(
    property?.id || property?._id || property?.title || "property",
  );

  const propertySeed = propertyKey
    .split("")
    .reduce((sum, char) => sum + char.charCodeAt(0), 0);

  const existingReviews = Array.isArray(property?.reviews)
    ? property.reviews
    : [];
    ...
    existingReviews.length > 0
      ? existingReviews.map((review, index) => ({
          ...
          userName:
            ...
            review?.name ||
            review?.guestName ||
            reviewNames[(propertySeed + index) % reviewNames.length],
        }))
      : Array.from({ length: 5 }, (_, index) => {
          const name = reviewNames[(propertySeed + index) % reviewNames.length];

          const text =
            demoReviewTexts[(propertySeed + index) % demoReviewTexts.length];

          return {
            id: `demo-${propertyKey}-${index}`,
            userName: name,
            name,
            rating: [5, 5, 4, 5, 4][(propertySeed + index) % 5],
            comment: text,
            text,
