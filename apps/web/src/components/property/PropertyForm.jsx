...
// Same multi-step wizard used for both host submissions and admin-created
// listings. `hostId`/`defaultStatus` decide who the property belongs to and
// whether it needs approval, `onSuccess` runs after the property is created.
const PropertyForm = ({ hostId, defaultStatus = 'pending', onSuccess, submitLabel = 'Submit Property', submittingLabel = 'Submitting...' }) => {
  ...
  const handleSubmit = async () => {
    if (!hostId) {
      toast.error('Missing host information, please try again.');
      return;
    }
    ...
    try {
      ...
      payload.append('hostId', hostId);
      payload.append('status', defaultStatus);
    ...
    <div className="space-y-8">
      ...
      <div className="bg-card border border-border rounded-3xl shadow-sm overflow-hidden min-h-[500px] flex flex-col">
        ...
        <div className="p-6 border-t border-border bg-muted/10 flex items-center justify-between">
          <Button type="button" variant="outline" onClick={prevStep} disabled={currentStep === 1} className="h-12 px-6 rounded-xl font-bold">
            <ChevronLeft className="w-4 h-4 mr-2" /> Back
          </Button>

          {currentStep < 3 ? (
            <Button type="button" onClick={nextStep} className="bg-primary hover:bg-primary/90 text-primary-foreground h-12 px-8 rounded-xl font-bold">
              Next Step <ChevronRight className="w-4 h-4 ml-2" />
            </Button>
          ) : (
            <Button type="button" onClick={handleSubmit} disabled={isSubmitting} className="bg-emerald-600 hover:bg-emerald-700 text-white h-12 px-8 rounded-xl font-bold shadow-md">
              {isSubmitting ? <><Loader2 className="w-5 h-5 mr-2 animate-spin" /> {submittingLabel}</> : submitLabel}
            </Button>
          )}
        </div>
      </div>
